...
These variable definitions are purely symbolic; they do not hold any data and cannot be used for computation. Calling [`StateVariables`](@ref) on a model, process, or [`Variables`](@ref) container (see following sections) results in corresponding [Fields](@ref) being allocated for each variable. 
