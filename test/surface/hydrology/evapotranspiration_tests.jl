...
# compute_evaporation_flux
#
# All evapotranspiration components share the functional form E = Δq · g, so a
# single set of tests covers ground/canopy evaporation and transpiration for
# both schemes.
# ==============================================================================

@testset "compute_evaporation_flux" begin
    @testset "$(nameof(typeof(evtr)))" for evtr in (
        ...
        @test iszero(compute_evaporation_flux(evtr, 0.0, g))

        # Flux is positive and equals Δq · g for positive Δq
        Δq = 0.001
        E = compute_evaporation_flux(evtr, Δq, g)
        @test E > 0
        @test E ≈ Δq * g

        # Flux scales linearly with Δq
        @test compute_evaporation_flux(evtr, 2Δq, g) ≈ 2E

        # Flux scales linearly with conductance
        @test compute_evaporation_flux(evtr, Δq, g / 2) ≈ E / 2

        # Negative Δq gives negative flux (condensation/deposition)
        @test compute_evaporation_flux(evtr, -Δq, g) ≈ -E

        # Numerical stability at extreme values
        @test isfinite(compute_evaporation_flux(evtr, Δq, 1.0e-12))
        @test isfinite(compute_evaporation_flux(evtr, Δq, 1.0e5))
        @test isfinite(compute_evaporation_flux(evtr, 1.0e-15, 1.0e5))
        @test isfinite(compute_evaporation_flux(evtr, 1.0, 1.0e5))

        # Type stability (Float32)
        E_f32 = @inferred compute_evaporation_flux(evtr, Float32(Δq), Float32(g))
        @test typeof(E_f32) == Float32
    end
end
...
@testset "Canopy ET flux" begin
    ...
    g_gnd = ground_evaporation_conductance(canopy_ET, β, rₐ, rₛ)
    ...
    @test iszero(ground_evaporation_conductance(canopy_ET, zero(β), rₐ, rₛ))
...
@testset "Physical consistency" begin
    ...
    @test compute_evaporation_flux(canopy_ET, Δq, g_trp_high_wind) >
        compute_evaporation_flux(canopy_ET, Δq, g_trp_low_wind)
    ...
    @test compute_evaporation_flux(canopy_ET, Δq, g_trp_open) >
        compute_evaporation_flux(canopy_ET, Δq, g_trp_closed)
    ...
    @test compute_evaporation_flux(canopy_ET, Δq, g_can_wet) >
        compute_evaporation_flux(canopy_ET, Δq, g_can_dry)
end
...
@testset "No vegetation scenario" begin
    ...
    @test iszero(compute_evaporation_flux(canopy_ET, Δq, g_can_no_veg))
end
...
@testset "Nighttime scenario (negative VPD)" begin
    ...
    E_trp_night = compute_evaporation_flux(canopy_ET, Δq_night, g_trp)
    E_can_night = compute_evaporation_flux(canopy_ET, Δq_night, g_can)
    @test E_trp_night < 0
    @test E_can_night < 0

    # Magnitude matches the corresponding daytime flux
    @test E_trp_night ≈ -compute_evaporation_flux(canopy_ET, -Δq_night, g_trp)
    @test E_can_night ≈ -compute_evaporation_flux(canopy_ET, -Δq_night, g_can)
end
