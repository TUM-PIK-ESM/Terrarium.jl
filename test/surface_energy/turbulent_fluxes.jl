...
@testset "Diagnosed turbulent fluxes" begin
    grid = ColumnGrid(CPU(), Float64, ExponentialSpacing(N=10))
    turbulent_fluxes = DiagnosedTurbulentFluxes(eltype(grid))
    seb = SurfaceEnergyBalance(eltype(grid); turbulent_fluxes)
    model = SurfaceEnergyModel(grid, seb)
    state = initialize(model)
    @test !hasproperty(state.inputs, :sensible_heat_flux)
    @test !hasproperty(state.inputs, :latent_heat_flux)
    ...
    compute_auxiliary!(state, grid, turbulent_fluxes, seb, model.atmosphere, model.constants)
    ...
    @test all(state.sensible_heat_flux .< 0)
    # TODO: Add test for coupling to ET
end
