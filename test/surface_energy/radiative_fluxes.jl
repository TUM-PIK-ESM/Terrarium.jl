...
@testset "Diagnosed radiative fluxes" begin
    ...
    @test hasproperty(state.auxiliary, :surface_longwave_out)
    ...
    @test all(state.surface_longwave_out .≈ (1 - 0.9)*surface_longwave_down + Terrarium.stefan_boltzmann(model.constants, 273.15, 0.9))
    @test all(state.net_incoming_radiation .≈ state.surface_shortwave_up - surface_shortwave_down + state.surface_longwave_out - surface_longwave_down)
end
