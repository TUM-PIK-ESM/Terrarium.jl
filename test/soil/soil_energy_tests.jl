...
@testset "Heat diffusion with periodic upper BC" begin
    ...
    model = SoilModel(grid; energy, strat, biogeochem, initializer)
...
@testset "Step heat diffusion" begin
    ...
    # this is just a hack to configure the model to simulate heat conduction in a fully solid medium
    soil_porosity = ConstantSoilPorosity(mineral_porosity=0.0)
    stratigraphy = HomogeneousStratigraphy(Float64; porosity = soil_porosity)
    model = SoilModel(grid; stratigraphy, biogeochem, initializer)
...
@testset "Thermal properties" begin
    ...
    @test Terrarium.thermal_conductivity(thermal_props, SoilVolume(porosity=1.0, saturation=1.0, liquid=1.0)) ≈ thermal_props.cond.water
    @test Terrarium.thermal_conductivity(thermal_props, SoilVolume(porosity=1.0, saturation=1.0, liquid=0.0)) ≈ thermal_props.cond.ice
    @test Terrarium.thermal_conductivity(thermal_props, SoilVolume(porosity=1.0, saturation=0.0, liquid=0.0)) ≈ thermal_props.cond.air
    @test Terrarium.thermal_conductivity(thermal_props, SoilVolume(porosity=0.0, saturation=0.0)) ≈ thermal_props.cond.mineral
    @test Terrarium.thermal_conductivity(thermal_props, SoilVolume(porosity=0.0, saturation=0.0, solid = MineralOrganic(organic = 1.0))) ≈ thermal_props.cond.organic
end
