...
@parameterized @kwdef struct PowerLawSnowThermalConductivity{NF} <: AbstractSnowThermalConductivity{NF}
    "Coefficient `a` in the thermal conductivity power law `κ = a·(ρ_s/ρ_w)^b`"
    @param conductivity_coefficient::NF = 2.22362 (units = u"W/m/K", bounds = Positive)

    "Exponent `b` in the thermal conductivity power law `κ = a·(ρ_s/ρ_w)^b`"
...
Bulk snow thermal conductivity via the density power law `κ_snow = a·(ρ_s/ρ_w)^b` ([yenReviewThermalProperties1981](@cite)).
"""
...
function compute_thermal_conductivity(
    ...
    ρ_s = ustrip(u"g/cm^3", ρ_s * u"kg/m^3")
    κ_s = 10^(a * ρ_s + b)
    return κ_s
end
...
function compute_thermal_conductivity(
    ...
    # convert density to g/cm^3
    ρ_s = ustrip(u"g/cm^3", ρ_s * u"kg/m^3")
    func = ifelse(ρ_s >= cond.threshold, cond.func_hi, cond.func_lo)
