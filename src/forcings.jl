...
"""
    forcing(i, j, k, grid, state, forcing::AbstractForcing, target::AbstractProcess, args...)
...
"""
@propagate_inbounds function forcing(i, j, k, grid, state, forcing::AbstractForcing, target::AbstractProcess, args...)
    return forcing(i, j, k, get_field_grid(grid), state.clock, state)
end

@inline function forcing(i, j, k, grid, state, ::Nothing, target::AbstractProcess, args...)
