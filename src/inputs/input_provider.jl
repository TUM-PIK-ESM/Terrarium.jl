...
Acts as an interface between a given `InputFields` container and one or more `InputSource`s
...
InputProvider(grid::AbstractLandGrid, sources::InputSource...) = InputProvider(InputFields(grid), collect(sources))
